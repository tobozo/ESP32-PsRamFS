 ...
 *
 * Optional args:
 *  - `targets`: a comma separated list of esp32 targets (e.g. esp32,esp32s3,etc...).
 *    Without this option all available targets for each idf versions are returned.
 *  - `extra_fqbns`: a comma separated list of FQBNS known to be EOL or preview (e.g. esp32@4.3.6,esp32@4.1.4)
 *    Without this option, only officially supported esp-idf versions are returned.
 *
 * Usage:
 *
 * $> php esp-idf-versions.php targets=esp32,esp32s2,esp32s3,esp32c6 extra_fqbns=esp32@4.4.6,esp32@4.3.6,esp32@4.1.4
 *
 * {
 *    "esp-idf-fqbn": [
 *        "esp32@5.2.1",
...
if (PHP_SAPI != "cli")
{
  php_die("Only CLI supported");
}

// hardcoded EOL versions, official versions will be added by the script.
$hardcoded_fqbns = []; // ['esp32@4.3.6', 'esp32@4.1.4'];

// FILTER IN: restrict output to these idf targets, other targets will be ignored
$idf_boards     = []; // ['esp32', 'esp32s2', 'esp32s3', 'esp32c6', 'esp32c3', 'esp32p4'/*, 'esp32h2', 'esp32c5'*/];


if( $argc>1 ) {

  echo sprintf("Got %d args".PHP_EOL, $argc);

  foreach( $argv as $id => $arg )
  {
    if( $id==0 ) continue;

    if( str_starts_with(trim($arg), 'targets=') )
    {
      // NOTE: captured esp32 targets are sanitized and will be verified.
      // Any invalid target captured here will be silently ignored
      if(preg_match_all("/[^\w](esp32((c|h|p|s)[0-9]{1,2})?)/", $arg, $matches))
      {
        if(!empty($matches[1]))
          $idf_boards = $matches[1];
      }
      continue;
    }

    if( str_starts_with(trim($arg), 'extra_fqbns=') )
    {
      // WARNING: captured FQBNS are sanitized but unverified.
      // Any version numbers captured here will be blindly forwarded and may fail much later if unmatched.
      if(preg_match_all("/[^\w](esp32((c|h|p|s)[0-9]{1,2})?@[\.0-9]+)/", $arg, $matches))
      {
        if(!empty($matches[1]))
          $hardcoded_fqbns = $matches[1];
      }
      continue;
    }
...
foreach($idf_versions_json['VERSIONS'] as $version)
{
  ...
  {
    if(empty($idf_boards) || in_array($board, $idf_boards)) // only keep supported targets
    {
      // e.g. esp32@5.2.1
...
// same as die() with with end of line and default errcode to 1
